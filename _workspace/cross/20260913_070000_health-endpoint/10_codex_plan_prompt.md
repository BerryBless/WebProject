너는 이 리포지토리의 구현 계획을 독립적으로 수립하는 역할이다. 다른 모델의 계획이 별도로 작성되고 있으며, 서로 교차 검증할 예정이다. 남의 결론을 추측하지 말고 네 판단만 작성하라. **`_workspace/cross/` 아래의 다른 파일(특히 `*claude*`)은 읽지 마라** — 읽으면 이 계획은 독립성 위반으로 무효 처리된다. 프로젝트 소스와 AGENTS.md만 참조하라.
