...
**아키텍처: 파이프라인(Plan→구현→리뷰) × 생성-검증(각 단계에서 Codex·Codex 상호 검증)**
...
핵심 불변식:
- **Codex는 실제 CLI 호출로만 참여한다.** Codex 에이전트가 Codex 역할을 흉내 내는 것은 금지. 모든 Codex 산출물 옆에는 `invoke-codex.ps1`이 남긴 `*.meta.json`(status=success)이 있어야 하며, 없으면 그 산출물은 무효다.
...
1. **실행 모드 판별:**
   ...
   - 미존재 → 초기 실행
2. **Codex 사전 점검 (필수):** `codex login status` 실행. 실패하면 **여기서 중단**하고 사용자에게 `codex login` 필요를 보고한다. Codex 없이 진행하며 "교차 검증"이라 부르는 것은 금지 — 사용자가 명시 승인하면 "Codex 단독 (교차 검증 아님)"으로 진행·보고한다.
...
1. **컨텍스트 작성** — 오케스트레이터가 `00_context.md` 작성: 사용자 요구사항(원문), 프로젝트 규칙 요약(AGENTS.md 관련 조항), 관련 코드 경로와 현재 구조 요지, 제약조건, 기준 커밋, 비범위.
...
| Codex **토큰/사용량 부족**(meta status=quota) 후 재시도도 실패 | 멈추지 않고 Codex 단독으로 작업을 완료한 뒤, 보고 첫머리에 "⚠ Codex 단독 수행 — 교차 검증 아님(Codex 토큰 부족)" 명시 (에러 핸들링의 폴백 절차) |
...
- **토큰 부족 폴백 (Codex 단독 저하 모드):** Codex 호출이 토큰/사용량 한도로 실패하고(meta `status=quota`) 재시도도 실패하면, 사용자를 기다리며 멈추지 않고 **남은 파이프라인을 Codex 단독으로 계속 진행해 작업을 완료한다.**
  1. 실패 시점부터의 Codex 단계는 스킵하고 Codex 산출물만으로 조정·판정한다 (이미 성공한 Codex 산출물은 유효하게 유지).
  2. run 디렉토리에 `DEGRADED.md`를 기록한다: 전환 사유(meta 원문 인용)·시점·스킵된 단계 목록.
  3. **최종 보고와 사용자 요약의 첫머리에 "⚠ Codex 단독 수행 — 교차 검증 아님 (Codex 토큰 부족)"을 명시**하고, 어느 단계까지 교차 검증됐고 어느 단계가 Codex 단독인지 구분해 피드백한다.
  4. 이 모드에서도 "교차 검증 완료" 문구는 금지다. 완료 조건 중 "양측 VERDICT"는 "Codex 측 VERDICT + 스킵 명시"로 대체된다.
- 충돌 의견은 삭제하지 않고 출처(Codex/Codex) 병기로 기록한다.
...
**에러 흐름 2 (리뷰 3라운드 초과):** High 지적 1건이 3라운드 후에도 유효/기각 미합의 → 쟁점 요약 + Codex 근거 + Codex 근거 + 선택지(수정안 A/B/보류)를 사용자에게 제시하고 대기.
