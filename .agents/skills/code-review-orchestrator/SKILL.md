...
description: "종합 코드 리뷰 오케스트레이터. 아키텍처·보안·성능·스타일 4개 에이전트 병렬 감사 후 단일 리포트로 통합. 트리거(리뷰 의도 명시 시): '코드 리뷰', '리뷰 해줘', 'PR 검토', '코드 감사', '종합 리뷰', '/code-review-orchestrator'. 후속: '다시 리뷰', '보안만 다시', '리뷰 보완'. 설명·수정·질문 요청에는 실행하지 않는다."
---
