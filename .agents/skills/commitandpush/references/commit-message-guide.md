...
- 메시지 끝의 **트레일러 블록**(빈 줄 뒤 `키: 값` 줄들)에 둔다. 런타임이 `Claude-Session:` 같은 다른 트레일러를 함께 넣을 수 있으며 순서는 상관없다
- 본문 있을 때: 본문 후 빈 줄 + 트레일러 블록
- 본문 없을 때: 제목 후 빈 줄 + 트레일러 블록
- 서명 문자열 자체는 수정 금지

## 자동 커밋 폴백 (Stop 훅 전용)

메인 세션이 메시지를 남기지 않은 턴에는 훅이 `{접두사}: 자동 커밋(메시지 미전달) — N개 파일 변경` 형식으로 커밋한다. 접두사는 파일 유형(문서·테스트·의존성·추가·수정)으로 고른다. 파일명 나열은 하지 않는다. 이 폴백 커밋은 `git-commit-writer`의 스타일 학습에서 **제외**한다.

## 저장 형식

`02_commit_message.txt`는 UTF-8 **BOM 없음**으로 저장하고 `git commit -F`로 전달한다. `-m "$(cat …)"`는 셸 인용·줄바꿈·BOM 문제가 있어 쓰지 않는다.
