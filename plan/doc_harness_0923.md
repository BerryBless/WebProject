...
| 12 | 수정 루프가 resume 시 회차 이력을 복원하지 않고 1회차부터 다시 시작(LLM 재검증 $30 반복) | 미해결 — 회차 영속화는 향후 과제. 완화: 사전 루프·발행 정책·반복 2회 |
...
- 수정 루프 회차 이력이 영속화되지 않아 resume 시 LLM 검증이 다시 든다.
...
1. 수정 루프 회차·이슈 영속화(`runs/<run>/loop.json`)로 resume 시 재검증 비용 제거.
