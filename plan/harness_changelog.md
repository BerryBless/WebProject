...
| 2026-09-25 | 실전 INITIAL 실행(run-0001)에서 잡은 결함 12건 수정: 한도 오류 즉시 실패·워커 중단·FAILED 재개, 안정 입력 해시(ISO 날짜·analysisStatus·Map 순서·하네스 diff 제외), 개수 고정 검증 조각, Mermaid·경로 검사 오탐(254+42건) 경고 강등·부정문 판정, 09_FEATURES 의존 다이어그램 표 폴백, 문서 이름 목록 주입, EISDIR, 결정적 사전 루프, `publish_on_residual` 발행 정책(잔여 지적을 19_UNKNOWN_AND_TODO에 기록), 스킬 실행을 분리 프로세스+Monitor로 | doc-harness/src·prompts·config, .claude/skills/doc-harness(+미러), .gitignore | `plan/doc_harness_0923.md` 7절. 사용량 한도 5회·캐시 무효화로 누적 $347 |
| 2026-09-25 | 실전 INCREMENTAL 실행(run-0002)에서 잡은 결함 5건 수정: LLM 검증 지적을 조각 안 문서로 한정, 색인 지적의 기능 재분석 확산 축소, TRIVIAL 변경은 직접 기능만 영향, 증분 이전 워크스페이스를 발행본(`workspace/current/`)에서 읽음, 교차 일관성 검사의 `GET, HEAD` 복합 메서드 분해(08_API 오탐 10건 → 수정 루프가 풀 수 없던 차단 제거) + 결정적 사후 검사(LLM 없이 싼 수정만) | doc-harness/src/verify/{consistency,llm,fixer,loop}.ts · src/change/impact.ts · src/pipeline.ts · scripts/repro-det.ts · test | 소스 한 줄 변경 → `문서화`가 기능 2개·문서 15개·다이어그램 3개만 갱신하고 baseline 2를 발행함을 실증하기 위해(잔여 LLM 지적 61건 기록 발행, 누적 $149). 상세 `plan/doc_harness_0923.md` 7절 |
| 2026-09-25 | 스킬 절차의 분리 실행 명령을 `npm.cmd`로 정정(`Start-Process -FilePath "npm"`은 Windows에서 Win32 오류로 실패, run-0002 재개 때 실측). 회차 영속화 과제는 Fake 러너 스파이크(중단→재개 시 LLM 재호출 2건)로 불필요 판정해 보고서에서 정정 | .claude/skills/doc-harness/SKILL.md · .agents/skills/doc-harness/SKILL.md · plan/doc_harness_0923.md · plan/resume_guide_0921.md(3b절 인계) | 다음 사람이 절차대로 실행했을 때 첫 명령부터 막히지 않게 하고, 틀린 과제에 시간을 쓰지 않게 하기 위해 |
