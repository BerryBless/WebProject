...
> **폐기됨(2026-09-20).** 제품 방향을 순수 기술 블로그로 바꾸면서 이 설계는 `plan/tech_blog_0920.md`로 완전 대체됐다. PARA 모델·노션 연동·"IP 화이트리스트만으로 쓰기 허용"은 더 이상 유효하지 않다. 이 문서는 결정 이력(특히 2.5절 Codex 검증에서 확정한 공통 규칙) 보존용으로만 남긴다. 8절의 구현 계획 파일은 삭제됐고 git 이력(`a669919`)에서 볼 수 있다.
...
- **영역 불변식(Codex 검증 반영).** `AreaId`는 "영역" 또는 "보관된 영역(`Kind=Archive && PreviousKind=Area`)"만 가리킨다. 보관된 영역도 유효한 소속 대상이다(과거 소속 사실 보존). 다른 항목이 참조 중인 영역은 Area가 아닌 종류로 바꿀 수 없다(400). 이 불변식은 검증→저장 사이 경쟁을 막기 위해 항목 생성·수정 트랜잭션에서 관련 영역 행을 `FOR UPDATE`로 잠근 뒤 검증한다. `PreviousKind`는 DB CHECK 제약(`Archive ⇔ PreviousKind ∈ {Project, Area, Resource}`)으로 손상을 원천 차단한다.
...
**2차 검증(2026-09-18, success):** 1차 반영은 전부 유효로 판정. 추가 반영 — (1) 테스트 팩토리 public 생성자 중복(xUnit 픽스처 실패) 수정, (2) Npgsql 마이크로초 절삭에 맞춘 `DbClock.UtcNow()` 단일 시각 출처, (3) 영역 종류 변경·자식 생성 경쟁을 트랜잭션 + 영역 행 `FOR UPDATE` 잠금으로 직렬화, (4) `PreviousKind` 불변식을 DB CHECK 제약으로 고정하고 손상 시 복원 409, (5) 참조 대상 삭제 경쟁(FK 23503)도 409, (6) 첨부 임시파일 정리 범위·루트 경로 검사 보강. 수용한 제한: offset 페이지네이션의 쓰기 중 중복·누락, 시그니처 판정은 이미지 디코딩 검증이 아님. 세부는 구현 계획 문서 끝의 반영표 참조.
