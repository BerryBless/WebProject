...
| D4 | `xmin` 행 버전 (`AppDbContext.cs:96` `IsRowVersion`, `PostEndpoints.cs:210,273`, `RenderedPostCache` 키) | 앱이 관리하는 `Version INT UNSIGNED NOT NULL`(**DB 기본값 없음**) + `IsConcurrencyToken()`. 삽입 시 값은 SaveChanges 인터셉터(`PostVersionInterceptor`)가 1로 넣는다 — 모든 Post 삽입이 EF를 거치고 원시 SQL 삽입 경로가 없어서다(구현 판정 Task 2: `HasDefaultValue`를 두면 EF가 삽입 후 재조회를 붙일 수 있다). **Posts 행이 바뀌는 모든 경로에서 +1**: SaveChanges 인터셉터(수정 상태의 Post) 그리고 `SeriesEndpoints.cs:206`의 `ExecuteUpdateAsync`(SeriesId를 비우는 경로)가 `SetProperty(p => p.Version, p => p.Version + 1)`을 함께 보낸다 | 타입이 `uint`로 유지되므로 SPA와 API 계약은 바뀌지 않는다. 시리즈를 풀었는데 버전이 그대로면 `RenderedPostCache`가 시리즈 내비게이션이 남은 오래된 HTML을 낸다(xmin은 자동으로 바뀌었다). 누락을 막기 위해 "Posts를 UPDATE하는 모든 코드가 Version을 올린다"를 아키텍처 테스트로 강제한다(`ExecuteUpdate` 사용처 소스 스캔 + 동작 테스트) |
| D5 | 권고 잠금 `pg_advisory_lock(hashtextextended('attachment:'+sha,0))` + `SET lock_timeout='10s'` (`Infrastructure/Storage/AttachmentLock.cs:48-80`) | `SELECT GET_LOCK(@name, 10)` / `SELECT RELEASE_LOCK(@name)`. 이름은 `att:` + `hex(SHA-256(DB이름))[..8]` + `:` + `sha[..48]`로 **최대 61자**(한도 64자) | ① MySQL 잠금 이름은 **서버 전역**이라 DB 이름으로 네임스페이스를 나눈다(테스트가 한 컨테이너에 DB를 여러 개 만든다). ② sha 앞 48자(192비트)로 자르면 충돌이 현실적으로 불가능하다. 설령 충돌해도 결과는 불필요한 직렬화뿐이라 안전하다. ③ 타임아웃이면 예외 대신 `0`을 반환하므로 `DbLockTimeoutException`을 직접 던져 503으로 매핑한다. `NULL`(오류)도 같은 경로로 보낸다. ④ 세션 잠금이라 연결을 명시적으로 여는 기존 구조를 유지한다. ⑤ 해제하지 않은 잠금은 `ConnectionReset=true`에서 **늦어도 같은 물리 연결이 다음에 대여될 때** 풀린다(스파이크 S6b 실측: 반납 직후 관측 `IS_FREE_LOCK` 0, 재대여 후 1 — 반납 시점 해제는 보장되지 않는다). 앱은 두 연결 모두에 `ConnectionReset=true`를 강제한다. ⑥ (최종 리뷰) PG의 `lock_timeout 10s`는 잠금 세션의 다른 잠금 대기도 끊었다 — 같은 통제로 `GET_LOCK` 직전 그 세션에 `SET SESSION lock_wait_timeout = 10`을 건다(메타데이터 잠금 대기 한정, 다음 대여 때 `ConnectionReset`이 원복). 경합한 `GET_LOCK`이 클라이언트 시간 제한에 먼저 끊기지 않도록 관리 연결의 `Default Command Timeout`은 0 또는 10초 초과여야 한다(시작 검증) |
| D6 | `SELECT … FOR UPDATE` (`SeriesEndpoints.cs:194-213`) | 그대로 쓴다 | D7의 격리 수준 고정이 전제다 |
| D7 | 기본 격리 수준 READ COMMITTED(PG 기본값) | **앱: `ReadCommittedTransactionInterceptor`**(`DbTransactionInterceptor.TransactionStarting`에서 격리 수준이 지정되지 않은 트랜잭션을 `BeginTransaction(IsolationLevel.ReadCommitted)`로 대체) + 서버 플래그 `--transaction-isolation=READ-COMMITTED`(compose·Testcontainers, **트랜잭션 밖 문장**의 격리 수준) | InnoDB 기본값 REPEATABLE READ는 갭 락과 넥스트 키 락 때문에 교착 양상과 `FOR UPDATE` 의미가 달라진다. 기존 동시성 추론은 모두 READ COMMITTED 기준이다. **인터셉터는 필수다(이중 방어가 아니다)**: MySqlConnector의 인자 없는 `BeginTransaction()`은 서버·세션 기본값과 무관하게 매번 `SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ`를 보낸다(스파이크 S14, general_log 실측 — 세션을 READ-COMMITTED로 맞춰도 트랜잭션 안에서 REPEATABLE-READ). 서버 플래그만 있으면 EF 트랜잭션이 전부 REPEATABLE READ로 돈다. 반대로 서버 플래그도 필요하다: 인터셉터는 `TransactionStarting`에서만 동작하므로, 명시 트랜잭션 밖에서 autocommit으로 도는 EF 쿼리·`ExecuteUpdate`/`ExecuteDelete`·원시 SQL은 세션 격리 수준을 따르고, `ConnectionReset=true`가 대여마다 세션 값을 서버 전역값으로 되돌리므로 그 전역값이 READ COMMITTED여야 한다. 둘은 서로 다른 문장을 맡는다. GitHub Actions 서비스 컨테이너는 명령 인자를 줄 수 없어 그 잡(web-e2e)에서는 트랜잭션만 READ COMMITTED가 보장되고 트랜잭션 밖 단일 문장은 REPEATABLE READ로 돈다(단일 문장의 일관 읽기라 E2E 결과에는 영향이 없다) |
...
| D10 | PG 기본 콜레이션(대소문자 구분) | 서버 `character_set_server=utf8mb4`, 기본 콜레이션 `utf8mb4_0900_ai_ci`. **식별자 열은 `utf8mb4_0900_bin`(NO PAD)**: Posts.Slug·Series.Slug·Tags.NormalizedName·Attachments.Sha256·Attachments.ContentType·Attachments.StoragePath | `ai_ci`(악센트·대소문자 무시)를 유니크 인덱스에 쓰면 PG에서 서로 달랐던 값이 충돌해 409가 된다(예: `cafe`/`café`). 태그의 대소문자 무시는 앱의 `NormalizedName`이 이미 담당하므로 DB는 이진 비교가 맞다. `utf8mb4_bin`이 아니라 `utf8mb4_0900_bin`인 이유: `utf8mb4_bin`은 PAD SPACE라 비교 전에 끝 공백을 무시해 `'abc'`와 `'abc '`가 유니크 충돌한다 — 바이트 비교는 NO PAD여야 성립한다(최종 리뷰에서 교체, 7절). 검색은 `ai_ci`라 PG의 ILIKE(악센트 구분)보다 넓게 맞는다. 이 차이는 수용한다(7절) |
...
- **EF Core 10 → 9 하향과 EF 9 지원 종료**: Pomelo 9.0.0이 EF 9에 묶여 있어 EF를 9.0.20으로 내렸다. 현재 코드가 쓰는 기능 중 잃는 것은 없다(스파이크 S16: EF 10 전용 API 미사용). 대가는 EF 10의 새 기능을 당분간 못 쓰는 것과 **지원 기간**이다: EF Core 9는 .NET 9에 묶인 STS 릴리스로, **지원 종료 예정 2026-11-10(.NET 9 STS 24개월 연장 기준 — 공식 지원 정책 페이지로 재확인 필요)**. 이 날 이후에는 보안 패치가 나오지 않는다. 대응: Pomelo의 EF 10 지원판 출시를 지켜보다 나오면 곧바로 올린다(패키지 3개와 `dotnet ef` 도구 버전만 바뀐다. D16 분류기와 고정 `ServerVersion` 덕에 영향 범위가 작다). 지원 종료 전까지 나오지 않으면 Oracle `MySql.EntityFrameworkCore`(EF 10) 스파이크(S1·S9)를 다시 돌려 전환 여부를 정한다.
...
- **첨부 잠금 세션의 메타데이터 잠금 대기 상한(최종 리뷰에서 복원)**: PG 판은 잠금 세션에 `lock_timeout 10s`를 걸어 그 세션의 모든 잠금 대기를 끊었다. MySQL 판 초기 구현은 `GET_LOCK` 대기만 10초로 묶고, 잠금을 쥔 세션의 이후 명령(행 삽입·삭제)이 메타데이터 잠금(예: 누군가의 `LOCK TABLES`·DDL)을 만나면 서버 기본 `lock_wait_timeout`(1년)까지 기다릴 수 있었다. `AttachmentLock.HoldAsync`가 `GET_LOCK` 직전에 `SET SESSION lock_wait_timeout = <대기 상한>`을 보내 복원했다(`AttachmentIntegrityTests.HoldAsync_BoundsMetadataLockWaitOnTheLockingSession`, SET을 지우면 31536000으로 실패). 남는 차이: InnoDB 행 잠금 대기는 여전히 서버 `innodb_lock_wait_timeout`(10초, 서버 인자)이 정한다 — PG의 `lock_timeout`처럼 한 값이 전부를 덮지는 않는다.
- **식별자 열 PAD SPACE → NO PAD(최종 리뷰)**: 처음 고른 `utf8mb4_bin`은 PAD SPACE라 `'abc'`와 `'abc '`를 같은 값으로 봐 D10이 의도한 바이트 비교가 아니었다. 운영 데이터가 없어 `utf8mb4_0900_bin`(NO PAD)으로 바꾸고 `InitialCreate`를 재생성했다(이전본과 차이는 콜레이션 문자열과 마이그레이션 ID뿐). 마이그레이션 ID가 바뀌었으므로 **이전 브랜치 빌드로 만든 로컬 DB 볼륨**은 "테이블이 이미 있다"로 기동이 실패한다 — 볼륨을 지우고 다시 만든다(운영 볼륨은 아직 없다). 증명: `CollationTests.TrailingSpaceVariants_AreDistinctUniqueValues`, `IdentifierColumns_AreBinaryCollated`(리터럴·`PAD_ATTRIBUTE='NO PAD'` 비교).
- **잠금 테스트의 `performance_schema.metadata_locks` 의존**: 잠금 대기 중인 요청을 관측하는 테스트(`AttachmentIntegrityTests`의 파일 소실 재저장 테스트)는 `performance_schema.metadata_locks`에서 이 잠금 이름의 PENDING 사용자 잠금 행을 폴링한다. 이 계측은 MySQL 8.4 기본값(`performance_schema=ON`, `wait/lock/metadata/sql/mdl` 계측 활성)에 기대므로, 테스트 서버에서 performance_schema를 끄거나 MDL 계측을 비활성화하면 그 테스트는 대기자를 영영 관측하지 못해 유계 폴링 시간 초과로 실패한다(거짓 통과는 아니다). 테스트 컨테이너 인자를 바꿀 때 이 두 설정을 건드리지 않는다.
