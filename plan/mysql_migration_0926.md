...
| EF 프로바이더 | **Oracle `MySql.EntityFrameworkCore` 10.0.9**. Phase 0 스파이크가 실패하면 Pomelo + EF 9로 후퇴한다 |
| 서버 | **MySQL 8.4 LTS** |
...
| 후보 | EF 10 지원 | 커넥터 | 판정 |
|---|---|---|---|
| **Oracle `MySql.EntityFrameworkCore` 10.0.9** | ○ (net10.0 그룹이 EF ≥ 10.0.9를 요구하므로 현재 고정값 10.0.12와 호환) | `MySql.Data` 26.7.0 | **채택**. 비동기 품질과 기능 지원은 Phase 0에서 실측한다 |
| Pomelo `Pomelo.EntityFrameworkCore.MySql` 9.0.0 | × (EF Relational `[9.0.0, 9.0.999]`에 묶임) | MySqlConnector 2.6 | 후퇴안. EF 전체를 9.x로 내려야 한다 |
| 둘 다 쓰는 추상화 | — | — | 기각(YAGNI) |

**Phase 0 go/no-go 기준**(하나라도 실패하면 Pomelo+EF9로 전환하고 이 문서를 개정한다):
1. `MySql.Data` 비동기 I/O가 스레드 풀을 막지 않을 것. 느린 쿼리(`SELECT SLEEP(2)`) 200건을 동시에 보내는 동안 스레드 풀 스레드 수와 대기 큐를 측정한다. sync-over-async면 스레드 수가 요청 수만큼 늘어난다.
...
### 2.4 오류 번호 대응(스파이크에서 실측 확정)

| 종류 (`DbErrorKind`) | PG SqlState (현재) | MySQL 오류 번호 (예상) | HTTP |
|---|---|---|---|
| UniqueViolation | 23505 | 1062 `ER_DUP_ENTRY` | 409 |
| ForeignKeyViolation | 23503 | 1451 (부모 삭제), 1452 (자식 삽입) | 409 |
| CheckViolation | 23514 | 3819 `ER_CHECK_CONSTRAINT_VIOLATED` | 500 (앱 검증 누락 = 버그) |
| QueryTimeout | 57014 | 3024 `ER_QUERY_TIMEOUT` | 503 |
| LockTimeout | 55P03 | 1205 `ER_LOCK_WAIT_TIMEOUT` + `GET_LOCK` 반환 0(D5) | 503 |
| Deadlock | 40P01 | 1213 `ER_LOCK_DEADLOCK` | 503 (또는 D8 재시도) |
| PermissionDenied | 42501 | 1142 `ER_TABLEACCESS_DENIED_ERROR`, 1044, 1227 | 500 |
| ReadOnly | 25006 | 1792 `ER_CANT_EXECUTE_IN_READ_ONLY_TRANSACTION` | 500 |
...
│  ├─ DataServiceCollectionExtensions.cs ← UseMySQL(D1)
...
└─ ApiFactory.cs             ← DB 생성 방식·풀 정리(MySqlConnection.ClearAllPools)
...
| 구분 | 파일 | 내용 |
|---|---|---|
| 패키지 | `Directory.Packages.props`, `PortfolioBlog.Api.csproj`, `PortfolioBlog.Api.Tests.csproj` | Npgsql EF → `MySql.EntityFrameworkCore` 10.0.9, `Testcontainers.PostgreSql` → `Testcontainers.MySql` 4.15.0. 전이 의존 고정 갱신 |
...
dotnet run --project _workspace/mysql-spike/Spike.csproj      # 결과: _workspace/mysql-spike/report.md
...
- **MySql.Data 커넥터 품질**: Phase 0에서 측정한다. 이후 회귀가 보이면 Pomelo의 EF 10 출시를 기다려 교체한다(D16 분류기 덕에 영향 범위가 작다).

**확장**
- Pomelo EF 10이 출시되면 프로바이더를 재평가한다(MySqlConnector는 비동기 품질과 `Connection Reset` 제어가 더 낫다).
