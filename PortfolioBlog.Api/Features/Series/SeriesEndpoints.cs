...
public static class SeriesEndpoints
{
    ...
    /// <param name="ct">요청 취소 토큰.</param>
    /// <returns>성공 시 갱신된 <see cref="SeriesDto"/>를 담은 200, 시리즈가 없으면 404, 검증 실패나 slug 변경 시도면 400.</returns>
    /// <remarks>
    /// <b>[성능 및 동시성 제약 조건]</b>
    /// <list type="bullet">
    /// <item><description><b>Thread Context:</b> ASP.NET Core 요청 파이프라인 스레드에서 호출된다.</description></item>
    ...
    /// slug는 요청에서 아예 제외하지 않고 값이 오면 현재 값과 비교해 다르면 400으로 거부한다(형식은 유효하되 변경은 금지).</description></item>
    /// </list>
    /// </remarks>
    ...
    /// <param name="ct">요청 취소 토큰.</param>
    /// <returns>성공 시 204, 시리즈가 없으면 404.</returns>
    /// <remarks>
    /// <b>[성능 및 동시성 제약 조건]</b>
    /// <list type="bullet">
    /// <item><description><b>Thread Context:</b> ASP.NET Core 요청 파이프라인 스레드에서 호출된다.</description></item>
    /// <item><description><b>Memory Policy:</b> 추적 엔티티를 로드하지 않는다. <c>ExecuteUpdateAsync</c>·<c>ExecuteDeleteAsync</c>는 변경 추적기를 거치지 않고 SQL을 직접 실행한다.</description></item>
    ...
    /// "글 UPDATE + 시리즈 DELETE"를 한 트랜잭션으로 묶는다. 시리즈가 없으면 <c>tx</c>를 커밋하지 않고 <c>await using</c> dispose에서 롤백해 방금 비운 글 UPDATE를 원복한다.</description></item>
    /// </list>
    /// </remarks>
