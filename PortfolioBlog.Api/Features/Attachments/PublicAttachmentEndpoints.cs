...
/// <item><description><b>Data Access:</b> 이 엔드포인트의 첨부 행 조회는 <see cref="PublicDbContext"/>(공개 조회 전용 연결 — <c>max_execution_time</c> +
/// <c>transaction_read_only=ON</c>, <see cref="PublicSessionInterceptor"/>)로 한다. 관리 풀을 쓰면 DB가 느릴 때 공개 요청이 시간 제한 없이 매달린다.</description></item>
...
public static class PublicAttachmentEndpoints
{
    ...
    /// <c>max_execution_time</c>(<c>Public:StatementTimeoutMs</c>)·<c>lock_wait_timeout</c>과 <c>transaction_read_only=ON</c>이 걸린다(<see cref="PublicSessionInterceptor"/>).</param>
