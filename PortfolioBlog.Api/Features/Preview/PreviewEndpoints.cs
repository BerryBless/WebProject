...
/// <item><description><b>Blocking:</b> 렌더링은 요청 스레드에서 동기 실행되는 취소 불가 CPU 작업이다(최대 수백 ms가 아니다). 코드 강조는
...
/// 이 방어의 일부가 아니다(클라이언트가 아무 제약 없이 직접 호출해도 서버가 막아야 한다).</description></item>
/// </list>
/// 본문은 로그에 남기지 않는다.
/// </remarks>
public static class PreviewEndpoints
{
    ...
    /// <param name="renderer">공개 페이지와 공유하는 마크다운 렌더러.</param>
    /// <returns>렌더링한 HTML을 담은 200, 검증 실패이거나 구조가 너무 깊게 중첩됐으면 400.</returns>
    /// <remarks>
    /// <b>[성능 및 동시성 제약 조건]</b>
    /// <list type="bullet">
    /// <item><description><b>Thread Context:</b> ASP.NET Core 요청 파이프라인 스레드에서 호출된다.</description></item>
    /// <item><description><b>Memory Policy:</b> <see cref="MarkdownRenderer.Render"/>가 만드는 HTML 문자열 1개를 응답 DTO에 실어 그대로 반환한다(서버에 보관하지 않는다).</description></item>
    /// <item><description><b>Concurrency:</b> Thread-safe. Blocking: 렌더링은 동기 CPU 작업이라(<see cref="MarkdownRenderer"/> 문서 참조) 이 메서드도 그동안 요청 스레드를 점유한다(취소 불가). 분당 한도·동시 실행 제한이 상위(<see cref="RateLimitingExtensions"/>)에서 스레드 풀 고갈을 막는다.</description></item>
    /// </list>
    /// </remarks>
    private static IResult Render(PreviewRequest request, MarkdownRenderer renderer)
    {
        ...
        // Features는 서로 참조하지 않으므로(전역 제약) PostEndpoints.TryRenderOrAddError를 재사용하지 않고 같은 확인을 여기서 다시 한다:
