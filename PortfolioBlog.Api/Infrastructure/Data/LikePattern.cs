...
/// <summary>사용자 입력을 <c>LIKE</c>의 "포함" 패턴으로 바꾼다. 메타문자(<c>\ % _</c>)를 이스케이프해 입력이 와일드카드로 해석되지 않게 한다.</summary>
/// <remarks>
/// <b>[성능 및 동시성 제약 조건]</b>
/// <list type="bullet">
...
/// </list>
/// MySQL의 기본 LIKE 이스케이프도 <c>\</c>지만 <c>NO_BACKSLASH_ESCAPES</c> SQL 모드에 대비해 항상 명시한다(스펙 D9).
/// 값은 항상 매개변수로 전달되므로 SQL 주입과는 별개의 문제다 — 여기서 막는 것은 "<c>%</c> 한 글자로 전체 테이블 스캔"과 의도와 다른 매칭이다.
/// </remarks>
public static class LikePattern
{
    /// <summary><c>LIKE</c> 호출의 <c>ESCAPE</c> 절에 넘길 이스케이프 문자.</summary>
    public const string Escape = "\\";

    /// <summary><paramref name="term"/>의 메타문자를 이스케이프하고 앞뒤에 <c>%</c>를 붙여 "포함" 패턴을 만든다.</summary>
    /// <param name="term">이스케이프 전 원본 검색어.</param>
    /// <returns><c>EF.Functions.Like(column, pattern, LikePattern.Escape)</c>에 바로 쓸 수 있는 패턴 문자열.</returns>
    /// <remarks>
    /// <b>[성능 및 동시성 제약 조건]</b>
    /// <list type="bullet">
