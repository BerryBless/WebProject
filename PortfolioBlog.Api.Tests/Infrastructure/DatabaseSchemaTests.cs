...
/// <summary>
/// 초기 마이그레이션이 적용하는 DB 스키마(테이블·CHECK·UNIQUE 제약·시드 데이터·xmin 매핑)를 실제 PostgreSQL 컨테이너로 검증한다.
/// </summary>
...
public sealed class DatabaseSchemaTests(ApiFactory factory) : IClassFixture<ApiFactory>
{
    ...
    // Postgres 예외의 SqlState만 필요한 여러 테스트가 "스코프 열기 → 변경 적용 → SaveChangesAsync → 실패 시 SqlState 추출"을
    ...
    /// <see cref="Post.Version"/>(xmin)이 0이 아닌지 검증한다.
    /// </summary>
    /// <remarks>
    /// <b>[성능 및 동시성 제약 조건]</b>
    /// <list type="bullet">
    ...
    /// <item><description><b>Memory Allocation:</b> <see cref="SaveAndGetSqlStateAsync"/> 호출당 스코프·<c>Post</c> 인스턴스 각 1개.</description></item>
