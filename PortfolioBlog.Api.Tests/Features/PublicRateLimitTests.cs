...
/// <b>[성능 및 동시성 제약 조건]</b>
/// <list type="bullet">
/// <item><description><b>Thread Safety:</b> <c>[Collection("mysql")]</c>로 같은 컬렉션의 다른 테스트 클래스와 <see cref="MySqlContainerFixture"/>(컨테이너 자체)를 공유하지만, 테스트마다 새 <see cref="ApiFactory"/>를 만들어 자체 DB(고유 데이터베이스명)와 자체 속도 제한기 상태(체인이 요청 스코프가 아니라 팩토리별로 새로 등록됨)를 가지므로 테스트 간 데이터·한도 간섭이 없다.</description></item>
...
/// </remarks>
[Collection("mysql")]
public sealed class PublicRateLimitTests(MySqlContainerFixture mysql)
{
    /// <summary>공개 자산 한도는 IP별이다: 같은 IP의 3번째는 429 + Retry-After, 다른 IP는 통과.</summary>
    [Fact]
    public async Task PublicAsset_IsLimitedPerIp()
    {
        using var factory = new ApiFactory(mysql, new Dictionary<string, string?> { ["Public:AssetPerIpPerMinute"] = "2" });
    ...
    {
        using var factory = new ApiFactory(mysql, new Dictionary<string, string?> { ["Public:AssetPerIpPerMinute"] = "1" });
        using var client = factory.CreatePublicClient();
    ...
    {
        using var factory = new ApiFactory(mysql, new Dictionary<string, string?> { ["Admin:UploadPerMinute"] = "1" });
    ...
    {
        using var factory = new ApiFactory(mysql, new Dictionary<string, string?> { ["Public:SearchConcurrency"] = "0" });
