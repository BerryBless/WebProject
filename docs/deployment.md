...
| `deploy/docker-compose.yml` | caddy·api·mysql + 백업/복원 전용 `tools`(profile). 전 서비스 `cap_drop: ALL`·`no-new-privileges`, api·caddy는 `read_only`, 네트워크 셋(`public` 게시·아웃바운드, `edge`·`db`는 `internal`). mysql 서버 설정(격리 수준·문자셋·`--local-infile=0`·`--secure-file-priv=NULL`·`--require-secure-transport=ON`)은 설정 파일이 아니라 compose `command` 인자로 준다 |
| `deploy/docker-compose.smoke.yml` | 스모크 전용 덮어쓰기(로그인 속도 제한 상향, `public` 서브넷 고정). 운영에는 쓰지 않는다 |
| `deploy/mysql-init/10-users.sh` | 빈 볼륨 최초 기동에만 실행. `blog` DB와 사용자 셋(`blog_app` 소유자 / `blog_public` 조회 전용, 둘 다 `REQUIRE SSL`)을 만든다. `root`는 `MYSQL_ROOT_HOST=localhost`로 컨테이너 안 소켓 전용이다(네트워크로는 로그인 불가) |
| `deploy/.env.example` | 도메인·오리진·허용 CIDR·`ACME_EMAIL`·사이트 표기·DB 비밀번호 셋(`MYSQL_ROOT_PASSWORD`·`BLOG_APP_PASSWORD`·`BLOG_PUBLIC_PASSWORD`)·관리자 해시. 실제 값은 `deploy/.env`(git 제외, 권한 600) |
| `deploy/backup.sh` · `restore.sh` | 무중단 백업(`mysqldump --single-transaction` → 첨부 tar, 자체 검증 + `SHA256SUMS`)과 복원(체크섬 확인 → 쓰기 쪽 정지 → DB 재생성 후 덤프 적용 → 첨부 원복 → 재기동) |
...
2B단계의 잔여 위험("공개 세션이 스스로 읽기 전용을 끌 수 있다")을 닫는 조각입니다. **구현 완료(MySQL 전환 후에도 유지).**

- `root`(운영자 전용, 컨테이너 안 소켓 전용) · `blog_app`(스키마 소유자, 마이그레이션과 관리 API, `blog.*` 한정 `WITH GRANT OPTION`) · `blog_public`(공개 조회) 셋으로 나눕니다. 앱은 **슈퍼유저로 접속하지 않습니다**.
- 공개 사용자의 권한은 init 스크립트의 일괄 부여가 아니라 **앱이 시작할 때마다** 다시 맞춥니다(`PortfolioBlog.Api/Infrastructure/Data/PublicRoleGrants.cs`): 허용 테이블 5개(`Posts`·`Series`·`Tags`·`PostTags`·`Attachments`)에만 `GRANT SELECT`를 준 뒤, **공개 연결 자신이 `SHOW GRANTS`로 자기 권한을 보고**하게 해 그 집합이 정확히 {`USAGE ON *.*`} ∪ {5개 테이블 SELECT}가 아니면 기동을 실패시킵니다(fail-closed). 허용 목록이 코드와 함께 버전 관리되고 테스트됩니다.
- 초과 권한은 **자동 회수하지 않습니다**(운영자가 원인을 보고 `REVOKE`, [운영 절차](../deploy/OPERATIONS.md) §12). 회수하려면 서버 출력 문자열을 SQL로 되돌려 조립해야 하고, 초과 권한이 생기는 경로는 사람의 수동 GRANT뿐이라 조용히 고치기보다 드러내는 편을 택했습니다.
- 회귀 가드: 통합 테스트가 컨테이너마다 독립된 공개 사용자로 `PublicRoleGrants.Apply`를 실행해 권한 집합이 항상 정확한지 매 실행마다 확인합니다.
...
2. **기동** — mysql·api·caddy가 healthy(또는 running, caddy는 healthcheck 없음).
...
7. **DB 사용자 권한** — `blog_app`·`blog_public` 둘 다 `SUPER`·`FILE`·`PROCESS`·`CREATE USER`·전역 `GRANT` 권한이 없고 TLS 필수(`ssl_type='ANY'`)인지, 서버 전역 설정(`local_infile=0`·`secure_file_priv=NULL`(8.4.11은 SQL NULL이 아니라 문자열 `NULL`로 보고하므로 둘을 구별해 검사하고, FILE 권한이 있는 root의 `INTO OUTFILE`이 1290으로 거부되는지도 확인)·`require_secure_transport=1`·`READ-COMMITTED`)이 실제로 적용됐는지, `root`가 `localhost`(소켓) 하나뿐이고 네트워크 로그인이 거부되는지, `blog_public`이 5개 허용 테이블만 읽고 쓰기·`AdminState`·마이그레이션 이력·`mysql.user`·`use mysql`·`SELECT ... INTO OUTFILE`(FILE 권한 없음, 1227)·비TLS 접속(계정에 `REQUIRE SSL`)은 전부 거부되는지, 서버 전역 `require_secure_transport`가 진짜로 작동하는지(임시 비-SSL 계정으로 별도 확인). 전부 컨테이너 네트워크 주소(`-h mysql`)로 접속해 실제 인증·TLS 경로를 탄다(소켓 접속이 인증을 우회하지 않는다).
...
| 컨테이너 헬스체크 | api는 `dotnet PortfolioBlog.Api.dll healthcheck`(`Host: <공개 호스트>`를 붙여 `/health` 호출), mysql은 `blog_app` 사용자로 `SELECT 1`을 실행(`mysqladmin ping`은 인증 실패에도 성공을 반환해 쓰지 않는다). **caddy는 헬스체크가 없다** — `docker compose ps`에서 `running`까지만 보인다(아래 잔여 위험) |

## 백업과 복원

- `mysqldump --single-transaction`(InnoDB 일관 스냅숏, 서비스 무중단)과 `attachments`를 **같은 시점에**(DB 먼저 → 첨부 나중). 내용 주소 파일은 덮어써지지 않으므로 그 사이의 어긋남은 "행 없는 파일"(청소 잡이 지움) 쪽으로만 납니다.
...
- ~~`blog_public`이 `postgres`·`template1` 데이터베이스에 CONNECT하고 임시 테이블을 만들 수 있다~~ **해소됨(MySQL 전환, R5).** MySQL은 PostgreSQL과 달리 권한이 DB 단위로 걸리고 `mysql`·`information_schema` 같은 시스템 DB에 `blog_public`이 접근할 수 있는 기본 PUBLIC 권한이 없다. 스모크가 `use mysql`·`select * from mysql.user` 거부를 확인한다(위 "배포 스모크" 7단계).
