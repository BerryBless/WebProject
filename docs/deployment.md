...
7. **DB 사용자 권한** — `blog_app`·`blog_public` 둘 다 `SUPER`·`FILE`·`PROCESS`·`CREATE USER`·`GRANT`가 없고 TLS 필수(`ssl_type='ANY'`)인지, 서버 전역 설정(`local_infile=0`·`secure_file_priv=NULL`·`require_secure_transport=1`·`READ-COMMITTED`)이 실제로 적용됐는지, `root`가 `localhost`(소켓) 하나뿐이고 네트워크 로그인이 거부되는지, `blog_public`이 5개 허용 테이블만 읽고 쓰기·`AdminState`·마이그레이션 이력·`mysql.user`·`use mysql`·`SELECT ... INTO OUTFILE`(FILE 권한 없음, 1227)·비TLS 접속(계정에 `REQUIRE SSL`)은 전부 거부되는지, 서버 전역 `require_secure_transport`가 진짜로 작동하는지(임시 비-SSL 계정으로 별도 확인). 전부 컨테이너 네트워크 주소(`-h mysql`)로 접속해 실제 인증·TLS 경로를 탄다(소켓 접속이 인증을 우회하지 않는다).
