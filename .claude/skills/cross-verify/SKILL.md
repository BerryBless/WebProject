---
name: cross-verify
description: "Claude와 실제 Codex CLI가 Plan·최종 리뷰를 교차 검증하는 개발 파이프라인. **요청에 '코덱스'/'Codex' 키워드가 명시된 경우에만 실행**('/cross-verify' 직접 호출 제외). 예: '코덱스 교차 검증으로 구현', 'Codex랑 같이 구현'. 후속: '코덱스 교차 검증 이어서'. 코덱스 언급 없는 '교차 검증'·'더블 체크'에는 실행 금지. 단발 Codex 질문은 codex 스킬."
---
