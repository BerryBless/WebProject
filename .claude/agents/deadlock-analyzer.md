...
## 입력/출력 프로토콜
- **입력**: `_workspace/concurrency-guard/00_input/source.txt`, 참고: `_workspace/concurrency-guard/02_lockfree_findings.json`
- **출력**: `_workspace/concurrency-guard/03_deadlock_analysis.json`
...
- **발신 (완료)**: `deadlock-reviewer`에게 `{"action": "review-requested", "output": "_workspace/concurrency-guard/03_deadlock_analysis.json", "iteration": N}` SendMessage
...
## 협업
- **lock-free-enforcer**: 락 위치 정보를 파일로 참조한다 (`_workspace/concurrency-guard/02_lockfree_findings.json`).
