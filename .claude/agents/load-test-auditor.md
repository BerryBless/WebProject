...
## 입력/출력 프로토콜
- **입력 1**: `_workspace/pipeline/02_io_loop/IoLoop.cs`
- **입력 2**: `_workspace/pipeline/02_dispatcher/ThreadDispatcher.cs`
- **입력 3**: `_workspace/pipeline/02_interface_contract.cs`
- **출력**: `_workspace/pipeline/03_load_test_audit.md`
- **스킬**: `/load-test-audit` 스킬로 감사 수행

## 팀 통신 프로토콜
- **수신**: `pipeline-supervisor`로부터 `{"action": "audit-requested", "artifacts": [...]}` 수신
- **발신 (완료)**: 감독자에게 `{"status": "done", "verdict": "APPROVE|BLOCK", "critical_count": N, "output": "_workspace/pipeline/03_load_test_audit.md"}` 전송
