...
## 검증 프로세스
1. `_workspace/concurrency-guard/03_deadlock_analysis.json`과 `_workspace/concurrency-guard/00_input/source.txt`를 함께 읽는다
...
## 입력/출력 프로토콜
- **입력 1**: `_workspace/concurrency-guard/03_deadlock_analysis.json` (분석기 보고서)
- **입력 2**: `_workspace/concurrency-guard/00_input/source.txt` (독립 검증용)
- **출력**: `_workspace/concurrency-guard/03_deadlock_review.json`
...
## 팀 통신 프로토콜
- **수신**: `deadlock-analyzer`로부터 `{"action": "review-requested", "output": "_workspace/concurrency-guard/03_deadlock_analysis.json"}` 수신
- **발신 (재분석 요청)**: `deadlock-analyzer`에게 `{"action": "reanalyze", "targets": [...], "reason": "..."}` SendMessage (최대 1회)
- **발신 (완료)**: 리더에게 `{"status": "done", "agent": "deadlock-reviewer", "output": "_workspace/concurrency-guard/03_deadlock_review.json", "final_score": N}` SendMessage
...
- **lock-justification-auditor**: `_workspace/concurrency-guard/02_lockjustification_findings.json`에서 락 위치 정보 참조 가능.
